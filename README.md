...
reach, and the support content can be Markdown, text, PDF, CSV, JSON or JSONL files.
...
Records with `question`/`answer` fields become `Q:`/`A:` pairs, so FAQ chunking keeps each
pair together. Every format goes through the same chunking, injection scan and RBAC tagging.
Other files (e.g. `.docx`) are listed as skipped when KB-4 is built.

Per-file settings: `title`, `tenant_id`, `trust`, and a `clearance_level` that can only
*raise* (never lower) the folder's clearance. Text files take them as YAML front-matter;
any file can use a sidecar instead, e.g. `prices.csv.meta.yaml` next to `prices.csv`.
...
data/                   # support content, one subfolder per document_type (md/txt/pdf/csv/json/jsonl)
...
2. **Detection knowledge bases**
   ...
   ```
3. **Support content** — add files (md, txt, pdf, csv, tsv, json, jsonl) under `data/<document_type>/`, then
   ```bash
...
- All issues from the code review in `update_info.md` are addressed; `pytest tests` → 70 passed.
...
- Scanned PDFs need OCR before ingestion; `.docx` and other formats aren't read.
