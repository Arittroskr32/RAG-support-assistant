...
diagrams. Users sign in with an email account whose role decides which documents they can
reach, and the support content can be Markdown, text, PDF, Word, Excel, CSV, JSON or JSONL
files.
...
Each folder under `data/` is one document type, and it can hold any number of files in any
mix of these formats (subfolders too):

| Format | How it's read |
|---|---|
| `.md` `.markdown` `.txt` | as text; Markdown headings become sections |
| `.pdf` | the text layer, one `Page N` section per page. Scanned PDFs (images only) have no text and are skipped with a warning; run OCR on them first |
| `.csv` `.tsv` | one record per row, as `column: value` lines (delimiter detected) |
| `.json` | a list of objects gives one record each; a single object gives one record; nested fields become `a.b: value` |
| `.jsonl` | one record per line (invalid lines are skipped) |
| `.docx` (Word) | paragraphs in document order; headings become sections, list items `- ` bullets, tables `column: value` records (first row = header); the document title comes from its properties |
| `.xlsx` `.xlsm` `.xls` (Excel) | every sheet; the first non-empty row is the header and each other row becomes a `column: value` record (formula results, not formulas; dates as `YYYY-MM-DD`). With several sheets, each sheet is a section, so citations show *Workbook > Sheet* |

Records with `question`/`answer` fields (in CSV, JSON, Excel) become `Q:`/`A:` pairs, so FAQ
chunking keeps each pair together. Every format goes through the same chunking,
...
---
```
PDF, Word, Excel, CSV and JSON files can't have that header, so put a small sidecar file next
to them, named after the file plus `.meta.yaml`:
```yaml
# data/sales_info/prices.pdf.meta.yaml
...
```
  skipped data/sales_info/scan.pdf: UnreadableDocument: no text layer (scanned PDF? it would need OCR)
  skipped data/sales_info/old.doc: unsupported type .doc (save it as .docx)
```
- Scanned PDFs (photos of pages) have no text layer. Run OCR on them first (e.g.
  `ocrmypdf scan.pdf scan_ocr.pdf`), then put the OCR'd PDF in the folder.
- Older formats (`.doc`, `.ppt`, `.pptx`, `.odt`, `.ods`, `.rtf`) aren't read; the message
  says what to save them as.
- A damaged file is skipped with the parser's error, instead of stopping the build.

### How a caller gets a role

The role and tenant always come from the server, never from the request:

1. **Browser account** (the chat UI). Click **Sign in / Register** and create an account
   with an email and password. It starts as `public`. To give an email another role, add it
   to `config/role_assignments.json`:
   ```json
   {
     "alice@example.com": "employee",
     "bob@partner.com": { "role": "partner", "tenant_id": "acme" }
   }
   ```
   The file is checked on every request, so a change applies right away, also to someone
   who is already signed in. An entry with an unknown role is ignored and logged, so a typo
   can only lower access. `config/role_assignments.example.json` shows the format.
   Passwords are stored as scrypt hashes in `config/users.json`. The session is an
   HMAC-signed cookie (HttpOnly, SameSite=Strict) that lasts `SESSION_TTL_HOURS`. Five
   wrong passwords in 15 minutes lock that email for the caller's IP for a while.
2. **API key** (`X-API-Key` header, for scripts and curl): `python -m api.create_api_key
   --user-id alice --role customer`. A key takes priority over a signed-in account.
3. **Neither**: role `public` (turn off with `ALLOW_ANONYMOUS=false`).
...
api/                    # FastAPI app (API + chat UI), accounts + cookie sessions, API-key auth
...
Set `GENERATION_BACKEND=anthropic` and `ANTHROPIC_API_KEY` to use Claude instead.

**GPU memory.** The guardrail takes ~1.7 GB in 4-bit. An 8B local model (~5 GB, Q4) fits next
to it on an 8 GB+ card. On a 4 GB card (e.g. GTX 1650) use a 3B model, otherwise Ollama runs
part of the 8B model on the CPU and answers get slow:
```bash
# .env
...
— accept its license, then `hf auth login` or `huggingface-cli login`) plus the LoRA adapter.
On a CUDA GPU it runs 4-bit quantized; on CPU it falls back to full precision (slow).
Everything else is CPU-friendly.

**PyTorch and the NVIDIA driver.** A PyTorch wheel only works if the driver supports its
CUDA version: `torch==2.14.0` from PyPI is built for CUDA 13 and needs driver 580+. With an
older driver (e.g. 550, CUDA 12.4) install the CUDA 12.6 build instead. `./run.sh` does this
...
2. Checks that PyTorch can use the NVIDIA GPU. It reads the driver's CUDA version from
   `nvidia-smi`, installs the matching torch build (`cu130` for CUDA 13 drivers, `cu126` for
   CUDA 12) if the installed one can't use the GPU, and stops with an error if CUDA still
   fails. It also warns when the chat model is too big for the GPU memory.
3. Wipes `chroma_db/` and the dataset split, and moves the last run's logs to `logs/previous/`.
4. Rebuilds KB-6, KB-5, KB-2 (when the datasets are in `data/security_datasets/`) and KB-4
   (files in `data/` that can't be read are listed as skipped).
5. Starts Ollama if it isn't already running (e.g. as a system service) and pulls the model
   on first use.
6. Checks the Hugging Face login for the gated guardrail model, then starts the server and
   waits until it's healthy.

Other options: `--calibrate` (re-calibrate thresholds; needs the datasets), `--port N`,
`--no-llm` (use a local LLM you manage yourself), `--cpu` (run without a working NVIDIA GPU;
the guardrail is then slow). `.venv`, downloaded models, `config/api_keys.yaml`,
`config/users.json`, `config/role_assignments.json` and `.env` are kept between runs. The first run installs the
...
2. **Detection knowledge bases**
   ...
   ```
3. **Support content** — add files (md, txt, pdf, docx, xlsx/xlsm/xls, csv, tsv, json, jsonl) under
   `data/<document_type>/`, then
   ```bash
...
4. **Calibrate** (joint, end-to-end FPR budget):
   ...
   ```
5. **Server, chat UI and roles**
   ```bash
   uvicorn api.main:app                                            # open http://localhost:8000
   ```
   The chat UI is served at `/`. Register under **Sign in / Register**; list emails that need
   another role in `config/role_assignments.json` (see *How a caller gets a role*). For
   scripts, create an API key instead:
   ```bash
   ...
   label), `GET /whoami` (the caller's user/role/tenant), `GET /health`,
...
- `logs/security_events.jsonl` — one record per request: request id, user/role/tenant, the
  ...
  checks, `echoed_tags` (context tags the model leaked and L7 removed), rich-output counts,
  per-layer latency. Queries are stored as SHA-256 + 40-char prefix.
...
- **Identity is server-side.** Role and tenant come from the signed session (looked up in
  `config/role_assignments.json` on every request) or the API key, never from the request body.
...
- All issues from the code review in `update_info.md` are addressed; `pytest tests` → 72 passed.
- Chat UI (`frontend/`) checked in headless Chrome against the mock server (light/dark,
  desktop/mobile, tables, charts, diagrams, blocked and error states, no CSP violations), and
  the account flow (register, role from the assignments file, sign out, wrong password,
  HttpOnly cookie) against the real API.
- Run end-to-end on a GTX 1650 (4 GB, driver 550): real MiniLM embedder, the L3 adapter on the
  GPU (blocks a direct injection), KB-2 from the datasets, and `llama3.2:3b` through Ollama.
  Not yet run with live Claude calls.
...
- Charts and diagrams are only as good as the local model's formatting; invalid blocks are
  replaced by a short note rather than rendered. Small (3B) models also sometimes add steps
  that aren't in the documents, which formatting checks can't catch.
- Greetings and questions nothing in `data/` answers ("who are you") get the escalation
  message: without relevant context the LLM isn't called at all.
- Chat history is kept per browser, not per account.
- Scanned PDFs need OCR before ingestion. Images inside Word files, charts and images in
  Excel, and older formats (`.doc`, `.ppt`, `.pptx`) aren't read.
